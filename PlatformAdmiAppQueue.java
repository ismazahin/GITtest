...
//class Application
public class PlatformAdmiAppQueue
{
    ...
    //methods if user enter code for Malaysia(Linked List)
    public static void RegisterCountryLL(String MCODE)
    {
         ...
         //initialize Queue
         Queue QueueG = new Queue();
         Queue GameQueue = new Queue();
         Queue GameQ1 = new Queue();
         Queue GameQ2 = new Queue();
         //initialize temporary queue
         Queue tempGame = new Queue();
         Queue tempGame2 = new Queue();
         //initialize counter
         int countAl=0;
         int countFree=0;
         int countPaid=0;
        ...
        try {
            ...
            while (scanFile.hasNext()) {
                ...
                //insert to queue without any condition
                GameQueue.enqueue(Cntry);
            ...
            if(ans==1)
            {
                    ...
                    while (!GameQ1.isEmpty()) {
                        Object Gdata = GameQ1.dequeue();
                    ...
                    while (!GameQ2.isEmpty()) {
                        Object Gdata2 = GameQ2.dequeue();
            ...
            {
                //Dequeue all data in queue while set a counter
                while (!GameQueue.isEmpty()) {
                        Object GCount = GameQueue.dequeue();
                        counter++;
                        tempGame.enqueue(GCount);
                    }
                //insert all data from temporary queue inside GameQueue
                GameQueue.enqueue(tempGame);
                while(!tempGame.isEmpty())
                {
                    tempGame.dequeue();
                }
                //
            ...
            else if(ans==3)
            {
                 ...
                 else if(ans3==2)
                {
                    ...
                    System.out.println("=========================================================================================================");
                    //Dequeue all data from QueueG to display list of games while also count row
                    int countdel=0;
                    while (!QueueG.isEmpty()) {
                        Object GSelRem = QueueG.dequeue();
                        if (GSelRem instanceof Country) {
                            Country Grem = (Country) GSelRem;
                            Grem.printReport2();
                            countdel++;
                        }
                        tempGame.enqueue(GSelRem);
                    }
                    System.out.println("=========================================================================================================");
                    ...
                    Country su = new Country();
                    su.setUnique(ansNoG);
                    
                    int count = 1;
                    boolean validGameNumber = false;
                    //if the row of the game that wanted to be delete is equal to counter,dont insert into QueueG
                    while(!tempGame.isEmpty())
                    {
                        Object data = tempGame.dequeue();
                        if(su.getUnique()!=count)
                            {QueueG.enqueue(data);}
                        else 
                    ...
                    if (su.getUnique() == 0) {
                        System.out.println("\n\tIndex out of bounds!");
                    } else if (su.getUnique() > 0 && su.getUnique() <= countdel && validGameNumber) {
                    ...
                    try
                    {
                         ...
                         while (!QueueG.isEmpty()) {
                            Object updatedGame = QueueG.dequeue();
                            if(updatedGame instanceof Country){
                                Country updatedCountry = (Country) updatedGame;
                                pw.println(updatedCountry.toString());
                            }
                            tempGame.enqueue(updatedGame);
                        }
                        
                       pw.close();
                       }//end try
                    catch(FileNotFoundException fnf)
                        {System.out.println("t/n" + fnf.getMessage());}
                        
                    catch(EOFException eof)
                        {System.out.println("t/n" + eof.getMessage());}
                        
                    catch(IOException io)
                        {System.out.println("t/n" + io.getMessage());}
                            
                    finally
                    {}
                    //display game list after deletion
                    System.out.println("\n\t**********New " +MCODE+" Games List**********");
                    System.out.println(String.format("\n|%-11s|%-11s|%-25s|%-11s|%-15s|%-25s|%-10s","No","Game Code","Game Name","Type","Price","Country Code","Developer"));
                    System.out.println("=========================================================================================================");
                    while (!tempGame.isEmpty()) {
                        Object GSelRem1 = tempGame.dequeue();
                        if (GSelRem1 instanceof Country){
                ...
                //user update selected game list
                 else if(ans3==4)
                 {
                    ...
                    //Dequeue all data from queue to display game list by row number
                    while (!QueueG.isEmpty()) {
                        Object GSelRem = QueueG.dequeue();
                        Country Grem = (Country) GSelRem;
                        Grem.printReport2();
                    ...
                    try
                    {
                         ...
                         BufferedWriter bw = new BufferedWriter(fw);
                         PrintWriter pw = new PrintWriter(bw);
                        //Dequeue all data in QueueG to print new data to txt file
                          while (!QueueG.isEmpty()) {
                            Object Gup2 = QueueG.dequeue();
                            Country Udp = (Country) Gup2;
                            pw.println(Udp.toString());
                            tempGame.enqueue(Gup2);
                        }
                       pw.close();
                       }//end try
                    catch(FileNotFoundException fnf)
                    ...
                    Object GSelRem1 = tempGame.getEnd();
                    Country Grem1 = (Country) GSelRem1;
                    Grem1.printReport();
                    }
                 }
